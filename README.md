...
| 위협 | 대응 |
|---|---|
| 금고 파일을 통째로 복사해 가는 경우 | 계정 이름까지 전부 암호문입니다. 봉인은 XChaCha20-Poly1305입니다. 마스터 암호를 쓰는 금고는 Argon2id(64MiB·3회)로 키를 만들고, 자동 해제 금고는 무작위 256비트 키를 쓰므로 경량 파라미터를 씁니다(탐색 공간이 2^256이라 사전 공격이 성립하지 않습니다). 암호 없이는 어떤 서비스를 쓰는지도 알 수 없습니다. |
...
| 로컬 소켓에 끼어들기 | 유닉스 도메인 소켓은 파일 권한 0600, Windows는 루프백 TCP(임의 포트)이며 루프백이 아닌 접속은 즉시 끊습니다. 더해서 요청마다 런타임 파일의 토큰(256비트)을 상수 시간으로 검사합니다. 런타임 파일은 유닉스에서 0600이고, Windows에서는 사용자 프로필 폴더의 ACL을 상속합니다. |
