...
- **단축키**: `⌘⇧O`(Windows·Linux는 `Ctrl+Shift+O`)로 마지막에 쓴 계정의 코드를 어디서든 복사합니다. 메뉴에서 끌 수 있습니다.
- **대시보드**: 계정이 많을 때 한 화면에 모아 봅니다. 검색과 남은 시간 표시가 있습니다. 메뉴의 "대시보드 열기"로 엽니다.
...
- **업데이트**: 서명을 검증한 뒤에만 설치합니다. 자리를 비운 사이에 알아서 설치하며, 쓰는 중에는 건드리지 않습니다.
