...
//!
//! 전송 계층은 유닉스 도메인 소켓(파일 권한 0600)과 Windows 루프백 TCP(임의 포트)다.
//! 여기에 더해 요청마다 런타임 파일에 적힌 토큰을 상수 시간으로 검사하므로, 소켓이나
//! 포트에 접근하더라도 토큰 없이는 아무 것도 얻지 못한다.
//!
//! 유닉스와 Windows의 차이를 분명히 해 둔다. 유닉스는 소켓 파일이 0600이라 다른 사용자는
//! 연결 자체를 못 한다. Windows는 같은 기기의 어떤 프로세스든 연결은 되고, 토큰만이 이를
//! 막는다.
