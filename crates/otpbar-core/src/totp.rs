...
pub fn decode_secret(secret: &str) -> Result<Zeroizing<Vec<u8>>> {
        ...
        .all(|c| c.is_ascii_uppercase() || ('2'..='7').contains(&c))
    {
        return Err(Error::InvalidSecret(
            "base32(A~Z, 2~7) 문자열이어야 합니다".into(),
        ));
    }
...
pub fn code_at(
    ...
    let modulus = 10u32
        .checked_pow(digits)
        .ok_or_else(|| Error::InvalidParams("자릿수가 너무 큽니다".into()))?;
    Ok(format!(
        "{:0width$}",
        bin % modulus,
...
mod tests {
    ...
    fn rfc6238_vectors() {
        ...
        for (secret, algo, t, expected) in cases {
            assert_eq!(
                code_at(secret, 8, 30, algo, t).unwrap(),
                expected,
                "algo={algo:?} t={t}"
            );
        }
    ...
    fn rejects_bad_params() {
        ...
        assert!(
            code_at(&s, 6, 0, Algorithm::Sha1, 0).is_err(),
            "주기 0은 거부"
        );
        assert!(
            code_at(&s, 20, 30, Algorithm::Sha1, 0).is_err(),
            "자릿수 20은 거부"
        );
        assert!(
            code_at(&s, 3, 30, Algorithm::Sha1, 0).is_err(),
            "자릿수 3은 거부"
        );
        assert!(
            code_at(&s, 6, 601, Algorithm::Sha1, 0).is_err(),
            "주기 601은 거부"
        );
    }
    ...
    fn rejects_bad_secret() {
        ...
        assert!(
            decode_secret("jbswy3dp ehpk-3pxp==").is_ok(),
            "공백·하이픈·패딩·소문자 허용"
        );
    }
}
